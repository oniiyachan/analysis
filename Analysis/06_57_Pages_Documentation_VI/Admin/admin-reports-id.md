# DOC CHI TIẾT TRANG: ADMIN REPORTS ID
- **Tệp Nguồn Code**: `src/app/(admin)/admin/reports/[id]/page.tsxpage.tsx`
- **Đường Dẫn URL**: `/admin/reports/[id]`
...
```mermaid
graph TD
    A([Truy cập trang]) --> B[Trình duyệt Gọi Page: /admin/reports/[id]]
    B --> C{Kiểm tra Cấp Quyền / Validator}
    C -->|Thành công| D([Gửi request lấy DB Server])
    C -->|Thất bại| E[Đá văng / Xử lý lỗi UI]
```
- **Mô Tả Chức Năng**: Giao diện cốt lõi (Core Engine) đứng ra fetch và hiển thị dữ liệu cho luồng định tuyến /admin/reports/[id].
