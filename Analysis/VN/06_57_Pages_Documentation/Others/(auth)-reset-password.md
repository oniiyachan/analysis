# DOC CHI TIẾT TRANG: (AUTH) RESET PASSWORD
- **Tệp Nguồn Code**: `src/app/(auth)/reset-password/page.tsxpage.tsx`
- **Đường Dẫn URL**: `/(auth)/reset-password`
...
```mermaid
graph TD
    A([Truy cập trang]) --> B[Trình duyệt Gọi Page: /(auth)/reset-password]
    B --> C{Kiểm tra Cấp Quyền / Validator}
    C -->|Thành công| D([Gửi request lấy DB Server])
    C -->|Thất bại| E[Đá văng / Xử lý lỗi UI]
```
- **Mô Tả Chức Năng**: Giao diện cốt lõi (Core Engine) đứng ra fetch và hiển thị dữ liệu cho luồng định tuyến /(auth)/reset-password.
